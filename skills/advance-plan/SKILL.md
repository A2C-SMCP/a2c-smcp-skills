...
| 结论 | 编排含义 |
|------|---------|
| **触协议** | 协议节点置**依赖图根**；所有代码节点 `blocked-by` 它。协议节点「完成」判据 = **已发布**（非仅 merge），由 `release` 收口。 |
...
## 依赖图 [ASCII，协议置根]
## 波次计划
- [ ] Wave 1: <协议节点> #<id>（release 发布后解锁）— 负责人 / 验收
...
| 节点类型 | 驱动 |
|---------|------|
| 协议节点 | `add-feature`（评审 → 合并）→ `release`（发布，解锁下游）|
| SDK / 客户端 Feature | `add-feature` |
| Bug 修复 | `fix-issue` |
| 新发现跨仓工作 | `issue-report` 建 Issue → 排入波次 |

每完成一个节点 → 回填母计划勾选。**前一波未收口不解锁下一波**（协议未发布禁 SDK 超前实现）。
...
| 协议节点未置根 / SDK 超前实现未发布协议 | 复用协议先行铁律，协议节点置根、发布后解锁 |
