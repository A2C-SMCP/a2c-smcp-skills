...
### 1.3 三仓态势扫描门控（SMCP 核心三仓强制）

当前项目属于核心三仓（a2c-smcp-protocol / python-sdk / rust-sdk）时，进入 Step 2 前**必须**按 `skills/issue-radar/SKILL.md` 执行三仓态势扫描：同主题是否已有人推进？在推方案与本次判断是否冲突（如默认名不同）？是否存在双向镜像 Issue？

- 🟢 无人推进 → 继续；🟡 有人推进且方案合理 → 转为挂靠，不重复开工
