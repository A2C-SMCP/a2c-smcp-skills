...
3. **配置不存在** → 提示用户交互式初始化：`! bash {baseDir}/scripts/smcp-env.sh init {mode}`
4. **单项更新** → `bash ... set {mode} <project> local <path>` 或 `... remote <ssh> <dir> <log>`
5. **remote 项目** → 调用 `mcp__ssh-mcp-server__list-servers` 验证 SSH 连接可用
