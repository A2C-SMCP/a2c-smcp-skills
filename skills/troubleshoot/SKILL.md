...
1. 运行 `bash {baseDir}/scripts/smcp-env.sh show {mode}` 查看当前配置
2. **配置存在** → `bash ... verify {mode}` 检查路径有效性，AskUserQuestion 确认是否需要更新
3. **配置不存在** → 根据 Step 1 识别的相关项目，使用 AskUserQuestion **逐个项目**向用户确认：
   - 该项目是否参与本次排查
   - 运行位置：local（提供本地路径）还是 remote（提供 SSH 连接名、远程目录、日志路径）
   - 每确认一个项目，立即用 `bash ... set {mode} <project> local <path>` 或 `... remote <ssh> <dir> <log>` 写入配置
   - 全部确认后 `bash ... show {mode}` 向用户展示最终配置
4. **remote 项目** → 调用 `mcp__ssh-mcp-server__list-servers` 验证 SSH 连接可用
