...
2. **配置存在** → `bash ... verify {mode}` 检查有效性，AskUserQuestion 确认是否需要更新
3. **配置不存在** → 使用 AskUserQuestion **逐项**向用户确认，每确认一个立即用脚本写入：
...
4. **remote 项目/Agent** → 调用 `mcp__ssh-mcp-server__list-servers` 验证 SSH 连接可用
