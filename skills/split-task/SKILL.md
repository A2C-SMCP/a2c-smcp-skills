...
| 结论 | 拆分含义 |
|------|---------|
| **涉及协议** | 必须切出一个**协议子任务**（落 a2c-smcp-protocol 或 oasp-protocol），置于**依赖图根**；所有代码仓库子任务（python-sdk / rust-sdk / office4ai / 客户端）**blocked-by** 它。协议子任务由 `/add-feature` 驱动（评审 → 合 develop + push → 发布）。**解锁开工** = 协议已合 protocol develop 且已 push origin（此后代码子任务可合入各自 develop）；**解锁合 main / 发版** = 协议已正式发布。 |
| **不涉及协议** | 纯代码拆分，按标准 main 可消费约束 |

> **硬门控**：协议子任务分两级完成判据——**develop 就绪**（合 develop + push，解锁下游开工）与**已发布**（解锁下游合 main / 发版）。这替换了该根节点的 Phase 3 判据。参见 `skills/add-feature/SKILL.md` 协议先行分级门控。
...
> 协议根子任务用 Phase 0.6 分级判据（develop 就绪 / 已发布）替代本表；代码子任务用下表。Story 内分解仅**编译期/类型期耦合**可豁免中间态破坏，末端 sub-task 必须拉回。
...
| **未超前于协议** | develop 实现不超前于 protocol develop（已 push）；合 main / 发版不超前于协议发布（A2C）|
...
[协议] a2c-smcp-protocol 事件新增 ──┬──→ [python-sdk] computer 实现 ─┐
  (develop+push 解锁开工，          └──→ [rust-sdk] smcp-computer 实现 ┼─→ [集成] 跨 SDK 序列化兼容守护
   发布解锁合 main/发版)
...
- [ ] **协议先行闭环**（如涉及协议）：协议子任务已在根、已达对应门级（开工 = develop 已 push；发版 = 已发布），代码子任务全部 blocked-by 且不超前
...
| 代码子任务超前于协议门级（未 push 就开工 / 未发布就发版） | 开工 blocked-by 协议 develop 已 push；合 main / 发版 blocked-by 协议发布 |
