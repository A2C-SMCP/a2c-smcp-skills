...
3. 如不涉及任何协议（纯 MCP 工具等），跳至 Step 5 直接实现
...
**目标**：协议发布后，确认哪些代码仓库需要跟进实现。

**动作**：

1. 根据 Step 0 判定的协议线，列出需要跟进的代码仓库
2. 确认各仓库的实现优先级和依赖关系
3. 为每个仓库创建 Feature Issue，引用协议 PR/版本号
...
Feature 需求
    │
    ▼
Step 0: 判定协议归属 ──── 无协议约束 ───→ Step 5: 直接实现
    │
    ▼ (涉及协议)
Step 1: 协议合规检查 ──── 现有协议已支持 ──→ Step 5: 直接实现
    │
    ▼ (需要协议变更)
Step 2: 协议变更设计
    │  - 向后兼容评估（与用户确认）
