...
**目的**：当前 Issue 若隶属于某个父 Issue（Epic / Tracking Issue / Parent），父 Issue 承载了整体设计意图、范围边界与验收标准。**后续的协议判定、变更设计、实现方案必须服从父 Issue 的北极星方向**，避免单个 Feature 偏离整体规划。
...
3. 提炼父 Issue 的"北极星要素"：
   - **整体目标**（这个大 Feature 要解决什么本质问题）
   - **范围边界**（哪些子能力属于本次范围、哪些被刻意排除）
   - **已定下的设计决策**（协议方向、架构选型、兼容性策略、依赖顺序）
   - **跨仓库协作约束**（SDK 先行还是客户端先行、版本对齐要求）
   - **验收标准**（父 Issue 层面的 Definition of Done）
4. 在后续 Step 0～Step 6 的每一次决策点，将方案与北极星要素对齐校验：
   - 协议归属判定是否与父 Issue 框定的协议线一致？
   - 协议变更设计是否在父 Issue 范围内，没有擅自扩张？
   - 代码仓库关联的依赖顺序是否符合父 Issue 约束？
5. **如果没有父 Issue**：记录"无父 Issue，以当前 Issue 自身为顶层范围"，跳过此子步骤
6. **如果父 Issue 与当前 Feature 方向冲突**：停止推进，使用 AskUserQuestion 向用户确认——是父 Issue 需要拆分/调整，还是当前 Feature 归属有误

> **硬性规则**：父 Issue 存在时，其已明确的整体设计决策优先级高于本 Feature 的局部直觉。如需偏离父 Issue 方向，必须显式与用户对齐，必要时先调整父 Issue。
...
Step 3: 协议变更设计
    ▼
Step 3.5: 消费方 cross-ask 验证 ◄── 强制门控
    │ (P0 反馈)
    └─→ 回 Step 3 修订
    ▼ (无 P0)
Step 4: 协议评审与发布 ◄── 硬性门控
