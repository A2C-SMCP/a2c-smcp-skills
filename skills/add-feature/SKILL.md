...
实现完成后，**禁止自评满意直接收尾**。拉起隔离上下文的 `code-reviewer` 子代理客观复审，按 `--review` 等级走 `/fix-review` 修复，🔴 未清零不算完成。子代理拉起时须传入「变更意图」（需求 / 验收标准 / 父 Issue 北极星），**不传**实现自评。
...
```
Feature 需求 → Step 0 判定协议归属 → Step 1 Issue 追踪治理
  ├─ 无协议约束 ─────────────────────→ Step 6 代码实现
